...
async fn output_collector_loop(state: Arc<AppState>, terminal_id: Uuid) {
    ...
    loop {
        ...
        if busy && !collector.interactive {
            if let Some(ref cmd) = pane_cmd {
                if crate::terminals::is_interactive_tui_command(cmd) {
        ...
        if !collector.interactive {
            let cap = tokio::task::spawn_blocking({
                let state = Arc::clone(&state);
                move || crate::terminals::capture_pane_for_terminal(&state, terminal_id)
            })
            .await
            .ok()
            .and_then(|r| r.ok())
            ...
            let delta = pane_text_since_baseline(&collector.baseline, &cap);
            let sanitized = sanitize_collected_output(&delta, &collector.command);
            if sanitized.len() > collector.last_output.len() {
                let new_part = &sanitized[collector.last_output.len()..];
                if !new_part.is_empty() {
                    let _ = patch_block(
                        ...
                        BlockPatch {
                            status: None,
                            content_delta: Some(new_part.to_string()),
                            content_replace: None,
                            meta: None,
                            exit_code: None,
                            finished_at: None,
        ...
        if busy && !collector.promoted && !collector.interactive && started.elapsed() > std::time::Duration::from_secs(2) {
            collector.promoted = true;
            let _ = state.auth.db().lock().update_terminal_block_kind(
                collector.output_block_id,
                "process_run",
            );
            let _ = patch_block(
        ...
        if !busy && started.elapsed() > std::time::Duration::from_millis(400) {
            let _ = patch_block(
                &state,
                terminal_id,
                collector.output_block_id,
...
fn interactive_session_output(
    ...
    sanitize_collected_output(
        &pane_text_since_baseline(&collector.baseline, &cap),
        &collector.command,
    )
}
...
fn sanitize_collected_output(raw: &str, command: &str) -> String {
    ...
    while lines.last().is_some_and(|l| is_shell_prompt_line(l.trim())) {
        lines.pop();
    }
    lines.join("\n")
}
